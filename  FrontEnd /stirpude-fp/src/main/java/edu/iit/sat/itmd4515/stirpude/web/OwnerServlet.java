...
import edu.iit.sat.itmd4515.stirpude.domain.Shift;
 ...
 * @author shruti
 */
//@WebServlet(name = "OwnerServlet", urlPatterns = {"/ow"})
...
public class OwnerServlet extends HttpServlet {
   ...
   // @PersistenceContext(unitName ="itmd4515") private EntityManager em;
    ...
    private static final Logger LOG = Logger.getLogger(OwnerServlet.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (request.isUserInRole("Owner_Role")) {
                Owner ow = owSvc.findByUserName(request.getRemoteUser());
                out.println("<h2>" + ow.getUser().getUserName() + ": " + ow.getName() + "</h2>");
                out.println("<h2>Date Hired: " + ow.getDateJoined() + "</h2>");
                out.println("<h2>Email: " + ow.getEmail() + "</h2>");
                out.println("<h2>Store Inventory: " + ow.getStoreInventory().getCustomerName() + "</h2>");
                out.println("<h2>Shift Information: </h2>");
                out.println("<ul>");
                for (Shift shift : ow.getShifts()) {
                    out.println("<li>" + shift.getShiftName() + "</li>");
                }
                out.println("</ul>");
            }
            out.println("</body>");
